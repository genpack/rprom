...
You can make any changes to the eventlog history of a `TranSys` object directly but you will need to 
...
we multiply the adjacency matrix 10 times recursively:
...
`TranSys` object has a method which computes the random walk probability distributions given the initial state and number of steps:
...
we can see which statuses cases are pulled towards as the step number increases.
...
The TranSys object has a method named `to_periodic` which converts itself into a periodic transition system and returns it as a new object.
...
In the next study, we will learn how to run a Monte-Carlo simulation to monitor the future of the process.
