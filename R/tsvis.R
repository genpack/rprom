...
plot.process.sankey = function(obj){
  list(nodes = obj$get.nodes(), links = obj$get.links()) %>%
    viserPlot(key = 'status', label = list(label = 'status'), source = list(source = 'status'), target = 'nextStatus', linkWidth = 'totalFreq', type = 'sankey', plotter = 'networkD3', config = list(node.label.size = 20, node.width = 25))
}
...
plot.statuses.bar = function(obj, measure = c('freq', 'totTime'), time_unit = c('second', 'minute', 'hour', 'day', 'week', 'year')){
  ...
  ND %>% arrange_(colname[measure]) %>%
    viserPlot(x = colname[measure], y = 'status', tooltip = 'tooltip', plotter = 'plotly', type = 'bar', config = cfg)
}
...
plot.statuses.box = function(obj, time_unit = c('second', 'minute', 'hour', 'day', 'week', 'year')){
  ...
  H %>% viserPlot(x = 'duration', y = 'status', plotter = 'plotly', type = 'box', config = cfg)
}
...
plot.cases.status.pie = function(obj, measure = 'time', aggregator = 'sum', time_unit = c('second', 'minute', 'hour', 'day', 'week', 'year'), trim = NULL){
  ...
  aa %>% viserPlot(theta = 'duration', label = 'status', config = list(legend.enabled = F), type = 'pie', plotter = 'plotly')
}
...
plot.cases.table = function(obj, time_unit = c('second', 'minute', 'hour', 'day', 'week', 'year'), plotter = 'DT', config = list(paging.length = 20)){
  ...
  tbl %>% viserPlot(label = cln, plotter = 'DT', type = 'table', config = config, filter = 'top')
}
...
plot.status.next.pie = function(obj, statusID, trim = NULL, plotter = 'plotly'){
  ...
  SN %>% na.omit %>% viserPlot(label = 'status', theta = 'freq', type = 'pie', plotter = plotter, config = list(title = 'Next Status Distribution'))
}
...
plot.status.prev.pie = function(obj, statusID, trim = NULL, plotter = 'plotly'){
  ...
  SP %>%  na.omit %>% viserPlot(label = 'status', theta = 'freq', type = 'pie', plotter = plotter, config = list(title = 'Previous Status Distribution'))
}
